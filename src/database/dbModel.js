...
// 前端数据模型类型识别规则（对标 asdm-aos 的 dataModelType）
// 用于在 code ontology builder 中标记 Interface/Class 的数据模型角色
export const FRONTEND_DATA_MODEL_RULES = [
  {
    type: 'API Response Type',
    detector: (entity) => {
      const name = entity.name || '';
      return /^(I|T)[A-Z].*(Response|Result|Data|VO|DTO|Info)$/.test(name)
        || /Response$/.test(name)
        || /Result$/.test(name);
    },
  },
  {
    ...
    detector: (entity) => {
      const name = entity.name || '';
      return /Request$/.test(name) || /Params$/.test(name) || /Payload$/.test(name);
    },
  },
  {
    type: 'Store Model',
    detector: (entity) => {
      if (entity.type === 'Store') return true;
      const name = entity.name || '';
      return /State$/.test(name) || /Store$/.test(name);
    },
  },
  {
    ...
    detector: (entity) => {
      const name = entity.name || '';
      // I + 大写字母开头的接口名（常见前端实体命名约定）
      return /^I[A-Z][a-zA-Z]+$/.test(name) && name.length >= 3;
    },
  },
  {
    type: 'Table Mirror',
    detector: (entity, tableNames) => {
      if (!tableNames?.length) return false;
      const candidates = codeNameToCandidateNames(entity.name);
      return candidates.some((c) => tableNames.includes(c));
    },
  },
];
