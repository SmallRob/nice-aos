...
description: |
  ...
  哪些函数没人调用 / 死函数 / 单例类有哪些 / 生成代码地图 / 依赖关系图 / 项目架构是什么样 / 有哪些功能模块 / 某目录的职责 /
  生成项目蓝图 / 领域蓝图 / 业务数据图 / 业务逻辑流向 / 可视化架构 / 脚本蓝图 / 脚本注入链 /
...
| **脚本蓝图 / 注入链可视化** | "看这个脚本的函数调用关系图" / "脚本注入链" / "谁注入到这个锚点" | `export --format html`（脚本蓝图页：SVG 调用图 + 注入锚点 + 网络端点）；数据层面 `query InjectionPoint` 看 `fns` 归属函数 |
...
| **函数级死代码** | "哪些函数没人调用？" / "死函数清单？" | `query Method --where "deadCandidate=true"`；类型级同理 `query Interface/Class --where "deadCandidate=true"`（保守判定） |
...
| **死代码候选** | "哪些文件没人用？" / "哪些类/函数是死代码？" | 文件级 `_meta.orphanCandidates`；类型级/函数级 `query Interface/Class/Method --where "deadCandidate=true"`；全量导出 Markdown 死代码候选节（三级） |
...
| ScriptFunction | 脚本函数/类/对象方法（含业务角色） | name（对象/类方法含 `.`，如 `storage.get`）, kind(function/arrow/class/object/method), owner, isTopLevel, roles(render/data/state/event/ui/logic，按函数内行为推断), line, lineCount, gmApiCount, domOpCount, networkCallCount, callCount, calledByCount, callIds, calledByIds, archLayer=script | `fn:` |
...
query Method --where "deadCandidate=true" --pretty  # 函数级死代码候选
...
# 循环依赖、死代码候选三级（文件/类型/函数）、Store 一览；
...
#   一图直读"入口函数 → 调用链 → 注入锚点/网络域"的逻辑注入链，利好油猴脚本开发者优化注入逻辑
...
```bash
query Method --where "deadCandidate=true" --pretty       # 死函数（非导出零引用 / 导出零导入）
query Interface --where "deadCandidate=true"             # 死接口
query Class --where "deadCandidate=true"                 # 死类
```

判定规则（宁可漏报不误报）：非导出实体本文件零引用（排除声明处与自递归）；导出实体全仓库零导入且本文件零引用；接口方法为契约声明永不判死。**人工复核要点**：仅被测试文件使用的导出符号会被误判（测试文件不入扫描范围）；命名空间导入 / `export *` / 动态 `import()` 的目标文件整体豁免。删除前先 grep 确认。
...
- 函数级死代码为保守判定（排除声明处/自递归；接口方法不判死）：仅被测试文件使用的导出符号会被判为候选（测试文件不入扫描），删除前人工 grep 确认
